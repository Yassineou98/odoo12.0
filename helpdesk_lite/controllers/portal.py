...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        ticket_count = request.env['helpdesk_lite.ticket'].search_count(['|', ('partner_id', 'in', ticket_access), ('partner_id', '!=', False)])
        values.update({
    ...
    class TicketForm(http.Controller):
        ...
        def ticket_form(self, **post):
            ...
            return request.render("helpdesk_lite.tmp_ticket_form", {
                ...
                'priorities': http.request.env['motifs'].fields_get('default_priority'),
            })
        ...
        def ticket_form_submit(self, **post):
            ticket = http.request.env['helpdesk_lite.ticket'].create({
                ...
                'attachment': post.get('attachement'),
                'type_id': post.get('type'),
                'motif_id': post.get('motif'),
                'priority': post.get('priorities'),
            })
